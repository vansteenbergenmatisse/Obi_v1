...
def parse_webhook_payload(payload: dict) -> EventEnvelope:
    ...
    event_type = payload.get("eventType") or payload.get("event") or payload.get("event_type") or ""
    page_id = _as_int(payload.get("page_id") or content.get("id"))
    cf_version = _as_int(payload.get("cf_version") or version.get("number"))
    space_id = _as_int(payload.get("space_id") or content.get("spaceId") or space.get("id"))
    status = payload.get("status") or content.get("status")
    actor_account_id = (
        payload.get("actor_account_id") or payload.get("userAccountId") or actor.get("accountId")
    )
