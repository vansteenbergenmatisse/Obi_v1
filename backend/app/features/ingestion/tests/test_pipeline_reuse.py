...
def _services(embedder: SpyEmbedder) -> IngestionServices:
    ...
    return IngestionServices(
        ...
        config=ChunkConfig(),
        contextualizer=Contextualizer(settings, client=None),
        embedder=embedder,
    )
...
def _child_plans(html: str):
    plan = plan_chunks(
        page_id=1001, blocks=norm.normalize_body(html), config=ChunkConfig(), counter=TokenCounter()
    )
...
def test_first_index_embeds_all_children() -> None:
    ...
    retrieval, embeddings = _resolve_children(
        _meta(), norm.normalize_body(HTML_V1), plans, [], _services(spy)
    )
