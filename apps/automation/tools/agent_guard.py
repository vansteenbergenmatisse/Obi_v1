...
Usage (from a hook):  python3 apps/automation/tools/agent_guard.py <profile>
...
PROFILES = {
    "auditor": {
        ...
        + [
            r"^(uv run )?python3? (apps/automation/)?tools/panel\.py\b",
            r"^uv run pytest\b",
    ...
    "tester": {
        ...
        + [
            r"^make\b",
            r"^(cd apps/automation && )?uv run (pytest|ruff|pyright)\b",
            r"^(uv run )?python3? (apps/automation/)?tools/panel\.py\b",
            r"^pnpm\b",
            r"^docker compose\b",
