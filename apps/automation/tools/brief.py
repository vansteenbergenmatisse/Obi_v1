...
ST = {'built': 'Implemented', 'change': 'Implemented, needs changing', 'build': 'Planned', 'discuss': 'Decision needed', 'unverified': 'Unverified'}
...
def svg_md(svg):
    ...
    for g in svg.find_all('g', attrs={'data-n': True}):
        ...
        boxes.append(f'  - {texts[0] if texts else "?"}' + (f' — {" / ".join(texts[1:])}' if len(texts) > 1 else '') + f' → panel `{g["data-n"]}`{chg}')
    labels = [t.get_text(strip=True) for t in svg.find_all('text', class_='lb') if t.parent.name == 'svg']
    out = ['Diagram (' + (svg.get('aria-label') or 'no description') + ')']
    if labels: out.append('  - labels: ' + '; '.join(labels))
    out += boxes
    return '\n'.join(out)
...
def block(node, depth=0):
    ...
    if name == 'h3':
        return '#### ' + node.get_text(strip=True)
    if name == 'p':
        if 'hint' in cls: return ''
        if 'cap' in cls: return '_' + inline(node).strip() + '_'
        if 'lead' in cls: return '**' + inline(node).strip() + '**'
    ...
    if name == 'summary':
        h3 = node.find('h3')
        if h3: return '#### ' + h3.get_text(strip=True)
        return ''
...
def panel_md(k, p):
    ...
    if p.get('n'): out.append(f'- Kind: {p["n"]}')
    if p.get('simple'): out.append(f'- In plain words: {p["simple"]}')
    if p.get('today'): out.append(f'- Today: {p["today"]}')
    if p.get('f'):
        out.append('- Settings and rules:')
        out.append('  | Setting | Value |\n  |---|---|')
        for a, b in p['f']: out.append(f'  | {a.replace("|","/")} | {str(b).replace("|","/")} |')
    if p.get('files'):
        out.append('- Where in the code:')
        for a, b in p['files']: out.append(f'  - `{a}` — {b}')
    if p.get('s'):
        out.append('- Steps:')
        for i, s in enumerate(p['s']): out.append(f'  {i+1}. {s}')
    if p.get('c'):
        out.append('- Code:\n```\n' + p['c'] + '\n```')
    if p.get('t'):
        out.append('- How to test it:')
        for t in p['t']: out.append(f'  - {t}')
    if p.get('note'): out.append(f'- Target and notes: {p["note"]}')
    return '\n'.join(out)

# panels grouped by section, in order of first appearance
order = []
for g in soup.find_all(attrs={'data-n': True}):
    if g['data-n'] not in order: order.append(g['data-n'])
for k in D:
...
md.append('')
md.append('Generated on ' + datetime.date.today().isoformat() + ' from `' + SRC + '` (the target design for Obi). This file carries every visible section, every table, every diagram box and every click panel of that page, in the page\'s order, so a reader who cannot open the HTML has the same information.')
md.append('')
...
for sec in soup.find_all('section'):
    ...
    if lede: md.append('**In plain words (the lede):** ' + inline(lede).strip() + '\n')
    wrap = sec.find('div', class_='wrap')
    parts = []
    for c in wrap.children:
        if isinstance(c, Tag) and (c.get('class') and ('sec-head' in c['class'] or 'lede' in c['class'])): continue
        x = block(c)
        if x: parts.append(x)
    md.append('\n\n'.join(parts))
    if by_sec.get(sid):
        md.append(f'\n#### Panels that belong to this section ({len(by_sec[sid])})\n')
        md.append(f'_Workflow label shown in the drawer: {WF.get(by_sec[sid][0].split("-")[0], "")}_\n')
        for k in by_sec[sid]:
...
for k in order:
    p = D[k]; sec = SEC.get(k.split('-')[0], '')
    md.append(f'| `{k}` | {sec} | {p.get("h","").replace("|","/")} | {ST.get(p.get("st"), p.get("st"))} |')
