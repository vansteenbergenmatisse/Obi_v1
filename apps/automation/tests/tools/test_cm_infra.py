"""Regression tests for design panel ``cm-infra`` — "infra/foundation: local Postgres".
...
repository root (outside ``apps/automation/``), so the repo root is located by walking up
...
# apps/automation/tests/tools/test_cm_infra.py -> parents[4] is the repository root.
REPO_ROOT = Path(__file__).resolve().parents[4]
COMPOSE_PATH = REPO_ROOT / "infra" / "foundation" / "docker-compose.yml"
