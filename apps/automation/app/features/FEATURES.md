...
- **Public surface (`__init__.py`, enforced):** `router` (POST /confluence/events), `run_reconciliation`, `drain`, `reap`, `KIND_LIGHTWEIGHT`, `KIND_COMPLETE`, `SlidingWindowRateLimiter`, `resolve_scope_roots`, `resolve_space_scope`, `ScopeResolution`, `ROOT_TYPE_SPACE`, `ROOT_TYPE_PAGE`. Everything else (event_service, sync_service handlers, per-job worker internals, reconcile_space) stays internal and is deep-imported only within the feature. Consumers import via `app.features.confluence_sync`; the boundary is checked by `tools/check_feature_boundaries.py` (ADR-0003).
...
- **Database tables owned:** `event_ledger`, `job`, `reconciliation_run`, `source_scope`. (It drives, but does not own, the `page_source`/`document`/`document_version`/`chunk` tables owned by `ingestion`.) `source_scope` rows are seeded one-off via `scripts/seed_source_scope.py` — no CRUD API yet.
...
- **Deployment unit:** ships in `apps/automation`.
- **Public surface (`__init__.py`, enforced):** `HybridRetriever` (the entry point; `retrieve()` for the eval-harness page-id shape, `retrieve_with_context()` for Phase 4.2's chunk id + score + title/url shape, `fetch_parent_texts()` for parent-context expansion), `PrincipalPermissionPolicy` (the policy callers construct), the `RetrievedHit`/`RetrievalResult` DTOs, and `update_query_trace_answer`/`update_query_trace_feedback` (the Phase 4/4.4 `query_trace` UPDATE writers). Fusion and the low-level search functions stay internal. Consumers import via `app.features.retrieval`.
- **Allowed importers:** the `rag_agent` answer workflow (Phase 4.2) and the evaluation harness. Imports downward into `ingestion` data (the `chunk` table) and `platform`.
- **Contracts / external systems:** the embedding provider (query embedding) via `platform.clients.embeddings_client`.
- **Database tables owned:** none — reads the `chunk` table owned by `ingestion`; owns the retrieval-write and answer-update paths on `query_trace` (schema owned by the 3.5.4 migration).
...
- **What it does:** The grounded answer runtime over the Phase 3.5 retrieval spine (ADR-0005). A fixed, testable workflow — conversational rewrite → RLS-scoped retrieve → RRF → cross-encoder rerank → **one CRAG corrective retry if weak** → refusal threshold → parent-context expansion → grounded generation with forced numbered citations → citation enforcement → SSE — **not** an agent loop. **Status: Phase 4.2 shipped** — `AnswerService` implements the full pipeline (minus SSE) and is unit-tested with fake LLM/retriever collaborators plus DB-integration-tested against the real fixture corpus (real retrieval, real parent expansion, real `query_trace` UPDATE; only the two LLM calls are faked, so the suite stays network-free). `POST /chat`, the principal-ACL store, and the real Anthropic wiring into `main.py` are Phase 4.3/4.4.
- **Path / owner:** apps/automation/app/features/rag_agent — owned by the `automation` application.
- **Language / framework / runtime:** Python 3.12; Pydantic v2 (DTOs), pure-Python domain (prompt assembly, refusal, citation enforcement), SQLAlchemy 2 only indirectly via `retrieval`. Runs inside the automation service.
- **Deployment unit:** ships in `apps/automation`.
- **Public surface (`__init__.py`, enforced):** the answer contract DTOs (`Answer`, `Citation`, `ChatMessage`); `AnswerService` (the Phase 4.2 orchestrator); the `QueryRewriter`/`AnswerGenerator` collaborator protocols and their `AnthropicQueryRewriter`/`AnthropicAnswerGenerator` implementations, so Phase 4.4 can wire real dependencies without reaching past this root. Prompt assembly / refusal / citation domain logic stays internal. Consumers import via `app.features.rag_agent`.
- **Allowed importers:** `app/main.py` (Phase 4.4 wiring of `POST /chat`). Imports downward into `retrieval` (`HybridRetriever.retrieve_with_context`/`fetch_parent_texts`, `update_query_trace_answer`) and `platform` (`AnthropicMessagesClient`); nothing imports back into it except `main`.
- **Contracts / external systems:** Anthropic messages (query rewrite via `routing_model`, grounded generation via `answer_model`) through `AnthropicQueryRewriter`/`AnthropicAnswerGenerator` — an LLM-CALL surface using `AnthropicMessagesClient`'s existing timeout/retry discipline; the rewriter fails open to the verbatim query on an `AnthropicError`, the generator lets it propagate (no safe fallback answer exists). `POST /chat` + `PATCH /chat/{trace_id}/feedback` (Phase 4.4) are the HTTP surface; the full `securing-http-and-llm-endpoints` control set (auth, rate limit, PII redaction, audit logging, cost caps) applies there, not to this internal workflow. The chat contract will be published in `packages/contracts`.
- **Database tables owned:** none — reads the `chunk` index (owned by `ingestion`) via `retrieval`, and updates `query_trace` (owned by `retrieval`) with the rewritten query, grounded answer, and citations once generation completes.
- **Untrusted input validation:** none at this layer yet — the Phase 4.4 endpoint owns request validation, rate limiting, and PII redaction. `AnswerService.answer` does validate its own precondition (raises `ValueError` if `history` is empty or does not end on a user turn). `enforce_citations` defensively strips any claim citing a page that was not retrieved; a generated answer with zero surviving citations degrades to a refusal rather than returning an empty response.
- **Shared components used:** None (backend feature).
- **Tests / risky paths:** `app/features/rag_agent/tests/` — refusal below/at/above threshold and the no-candidates case; citation enforcement (keeps cited claims, strips uncited and hallucinated-source claims, drops invalid markers, dedupes used markers); pure prompt assembly (rewrite prompt, evidence block numbering/ordering, missing-parent-text degradation); `AnswerService` orchestration with fakes covering rewrite-enabled/disabled, CRAG retry succeeding/still-weak/disabled, no-candidates refusal, and the no-grounded-claim refusal. `confluence_sync/tests/test_answer_workflow.py` — the same service end-to-end against the real indexed corpus (real RLS/source scoping, real parent join, real trace persistence).
- **Run / test:** `pytest app/features/rag_agent/tests app/features/confluence_sync/tests/test_answer_workflow.py`.
