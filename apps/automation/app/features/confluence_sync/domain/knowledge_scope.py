...
(anything other than ``obi-general-test``) on the same page is a conflict — a page can only live
in one provider's knowledge scope — so it contributes zero label-derived tags until an operator
fixes the Confluence labels; the caller unions the (possibly empty) result with the page's existing
...
def resolve_knowledge_scope_tags(
    ...
    provider_tags = tuple(tag for tag in matched if tag != "obi-general-test")
