...
exact same ``app.platform.db.schema`` primitives and connection pattern as
...
def _add_page_chain(session: Session, *, page_id: int, dim: int, source_id: str) -> None:
    ...
    (see the ``Document.page_id`` docstring reference in ``app/platform/db/models.py``): the ORM's
