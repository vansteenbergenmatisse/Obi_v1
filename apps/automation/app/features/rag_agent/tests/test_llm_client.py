...
def test_r1_small_reply_uses_the_configured_model_capped_by_max_tokens() -> None:
    """panel r1-small, check (b): the reply is generated by the model the generator was
    constructed with — today that is `settings.answer_model` (wired in `main.py`), not
    `settings.routing_model` (Haiku) — and capped by `_SMALL_TALK_MAX_TOKENS`. This pins today's
    actual wiring so a silent model change in either direction shows up as a failing test."""
    seen: list[dict] = []
    generator = AnthropicAnswerGenerator(_client_capturing(seen), "answer-model")
