...
- **Exported symbols (`index.ts`):** `ChatPanel` (the feature's root React component, rendered by the chat route) and the view-model types `ChatMessage`, `MessageRole`, `MessageStatus`. Internals (`ui/`, `api/`, `model/`) are private behind this root.
- **Internal structure:** `ui/` (`chat-panel`, `message-list`, `composer` — feature-only components), `api/` (`chat-client` — browser fetch wrapper over `/api/chat`), `model/` (`messages` — render view-model). Streaming/SSE parsing is stubbed until Phase 4.
