...
## Phase-4 embed security-hardening pass — S3: backend identity namespacing + defense-in-depth (2026-09-22)

Three backend-side gaps. The substantive one (CIP-4): the token subject used to key the rate-limit bucket, the idempotency cache, the answer cache, and the audit fingerprint was the raw JWT `sub`, with no issuer namespacing. Two different trusted issuers that both minted a token with the same `sub` would collide across all four — no content leak in v1 (those keys also carry the scope set and a null principal), but a latent cross-tenant collision and, today, real cross-issuer rate-limit interference and ambiguous audit attribution. The fix namespaces the identity key as `issuer\x00subject`, using a NUL separator that cannot appear in a JSON claim value, so no crafted `sub` can forge a different issuer/subject split. The tokenless path keeps a null subject. All four consumers treat the value as opaque, so the change preserves and strengthens distinctness without altering their logic. The two smaller items: a build-time `server-only` trip-wire on the automation-api client so the host key's reader can never be imported into a browser bundle (it could not leak today, but this makes an accidental import fail the build), and — for the page-principal ACL, which in v1 is enforced only in application code with no DB backstop — no code change but a new ADR (0019) recording that as a known, accepted defense-in-depth gap and gating a mandatory RLS backstop before per-principal ACL is ever enabled, plus a regression test pinning the `principal=None → only unrestricted pages` invariant it rests on.

Two things surfaced during the main-window verification that are worth stating plainly. First, an implementer miss: the agent ran only the `rag_agent` test tree, so it did not catch that the identity-key change deterministically broke an audit-fingerprint test living under `confluence_sync/` (`test_chat_endpoint.py:770`), which asserted the hash of the raw subject. Running the full DB suite in the main window surfaced it; the assertion was updated to the namespaced fingerprint (a test encoding the old behavior corrected to the new, not a weakening) and re-verified. This is exactly why the loop verifies independently of the implementer rather than trusting the report. Second, a pre-existing flake was discovered and is explicitly not being papered over: an ingestion DB test (`test_versioning_failed.py`) intermittently fails in the full suite with a chunk-staging error, but passes in isolation and passed on the final full run, and the failing test varied between runs — a shared-DB-state / rollback-isolation flake in the ingestion tests, unrelated to any embed-security slice (none touch ingestion or DB rollback). It is recorded for the independent audit wave to confirm, since it contradicts the matrix's earlier "deterministic fixtures" reading; it did not gate this slice.

Verified, test-first for CIP-4: backend unit 538→543 passed, the full backend DB suite 271 passed / 1 xfailed after the audit-test fix, knowledge-base 17 unit + 18 DB passed, frontend vitest 253 and typecheck clean, ruff and pyright clean on every touched file, boundaries green.

## Phase-4 embed security-hardening pass — S4: migration-0010 deploy-readiness gate (2026-09-22)

Migration 0010 installs the RESTRICTIVE customer-scope RLS that is Obi's fail-closed tenant backstop, and ADR-0014 frames it as the gate before a public deploy — but nothing enforced it. A real deployment on a database where 0010 was never applied (or where a downgrade dropped the policies) would expose the reader path with the DB-level backstop silently absent, leaving cross-integration isolation resting on the application layer alone. This slice adds the missing enforcement: a read-only deploy-readiness check, added as a `check-deploy-readiness` phase on the existing operator CLI (`setup_supabase.py`, which already owns the preflight/provision/verify-isolation family). It reads the Postgres catalog and asserts, for both scope policies, that the table exists, the policy is present as RESTRICTIVE and FOR SELECT, and row security is enabled; on any gap it prints exactly what is missing and refuses — "do not expose the reader path until `alembic upgrade head` (>= 0010) is applied" — and exits non-zero. It reveals no secret and invents no connection string, reading the target DSN the same way the existing phases do.

One correctness call worth recording: the check asserts RLS is ENABLEd, not FORCEd. ADR-0013 deliberately keeps the `chunk` table at ENABLE + NO FORCE (an owner-role exemption on managed Postgres), and ENABLE is precisely what binds the non-owner reader role to the RESTRICTIVE policy — so requiring FORCE would contradict ADR-0013. The implementer made this call and it was verified against the ADR.

The 0010 status the mandate asks to be recorded is now written down in a new operator runbook (`docs/runbooks/deploy-readiness-scope-rls.md`): 0010 runs unconditionally everywhere, is merely inert locally (local reads use the RLS-exempt owner role, so the demo does not rely on DB-level isolation), and is mandatory — with this gate passing — before any real deployment serves `/chat` through a non-owner reader role. No new ADR was created, since ADR-0014 already owns the decision; this slice only adds the enforcing check and the runbook.

Two DB-marked tests prove it, both rolled back: ready-when-the-policies-are-present (rc 0), and fails-closed-when-they-are-dropped (non-zero, naming each missing policy). Verified: the two tests pass, `make test-unit` stays green (backend 543, knowledge-base 17, boundaries clean), ruff and pyright clean on the touched files.

## Phase-4 embed security-hardening pass — S5: regression-test coverage (2026-09-22)

The final implementation slice adds twelve regression tests and changes no production code — the behaviors were already correct; these lock them in so a future change can't silently regress them. The important ones prove tenant isolation end-to-end rather than by construction: an end-to-end `/chat` test drives a Mews token through the real reader-role retrieval and asserts no Toast page or chunk appears in the trace or citations while the Mews content that should be visible is cited — with a Toast-token control in the same test proving the Toast page is genuinely retrievable for that query, so the test can't pass by returning nothing. The rest cover the forwarded-body-principal being inert, distinct token subjects never sharing a cache entry, the JWKS client being cached with a finite timeout, missing-`sub`/missing-`exp` token rejection, a body principal being unable to split the rate-limit bucket, and two frontend negatives — the token never appearing in a URL or history entry, and the JWT never appearing in any console channel. The meaningfulness of the two isolation tests was confirmed with a throwaway probe (since deleted): a Mews scope returned only the Mews page, Toast only the Toast page, and the wildcard both — so a broken isolation would put the Toast id in the Mews trace and fail the test.

One finding is recorded rather than fixed, because fixing it would mean changing production behavior the design intends. Parent-text fetch (`fetch_parent_texts`) deliberately widens the scope GUC to `'*'` (ADR-0014), so the RESTRICTIVE scope policy does not re-filter parent chunks. A parent carrying a *different* scope tag than its authorized child therefore is surfaced to the generator. This cannot arise through normal ingestion — a page's parent and child share that page's tags — and is safe because expansion only ever runs on child ids that already passed the scoped search (which the sibling test locks). The literal "must not surface" assertion would fail against current code, so the actual documented behavior was pinned and the theoretical concern flagged for the independent audit wave's authorization reviewer to weigh. No new pyright errors were introduced; the two pre-existing token_verifier and six pre-existing chat-endpoint protocol errors are unchanged, consistent with the no-rise floor.

Verified: backend unit 543→548, full backend DB suite 278 passed / 1 xfailed (the pre-existing ingestion flake passed this run), knowledge-base 17 unit + 18 DB, frontend vitest 253→256 and typecheck clean, ruff and format clean on all touched files. This closes the implementation phase (S1–S5); the first fully independent audit wave — fresh agents that never saw the implementation, trying to break each control — follows.

## Phase-4 embed security-hardening pass — Wave-1 audit + repair R1: fail-closed trust boundary (2026-09-22)

The first independent audit wave (six adversarial auditors, each finding then independently verified) returned fifteen confirmed findings and refuted one. The most important — and a genuine miss in my own SEC-S2 work — is that the production platform-trust guard was fail-open: `Settings.env` defaulted to `local`, so a deployment that simply forgot to set `ENV` was treated as offline and silently skipped both the "reject test/localhost/placeholder platforms" guard and the "at least one active platform" guard. A security boundary must fail closed. Repair R1 inverts the default: `env` is now empty by default, an unset or unknown environment is treated as production, and relaxation requires an explicit `local`/`test`/`ci` opt-in. The offline set was narrowed to drop `dev`/`development` (so a dev or staging deployment is guarded, not exempt), and the `allow_empty_platforms` escape hatch — which previously could disable the guard on its own — is now gated so it only takes effect in an offline env. The proof that it fails closed: booting with `ENV=production` against the committed registry now raises, because `datahub` is still an active `TODO` placeholder (which is exactly the deploy-time blocker already recorded for the owner).

This was high-blast-radius and handled honestly rather than by loosening anything. The test harness relied on the old `local` default, so the conftest now sets an explicit test env; and four pre-existing tests that constructed a production `Settings` only passed before because the `allow_empty_platforms` bug masked the guard — they were relying on the very defect being fixed, so each was given a valid production platform registry to test what it actually intends (client factories raising without a key), not weakened. One stale cross-reference comment in a frontend test file is left for the frontend repair slice, where it belongs.

Verified independently, TDD (the six new tests failed first): backend unit 555 passed, full backend DB suite 278 passed / 1 xfailed (the pre-existing ingestion flake passed again), knowledge-base 17 unit + 18 DB, frontend vitest 256 and typecheck clean, ruff and pyright clean on the touched files, boundaries green — plus a direct check that a production boot against the committed registry raises.

## Phase-4 embed security-hardening pass — repairs R2 (backend gating) + R3 (frontend CSP) (2026-09-22)

Two more wave-1 findings closed on the backend authorization boundary. First, inactive platforms were not actually gated: the design documents a two-tier model — verify any registered issuer's signature, but only *serve* active ones — yet the serving gate had no call sites, so a deactivated platform's valid token still got full scoped access. `build_auth_context` now resolves the issuer through the active view and refuses an inactive platform outright, even on the general-only path. Second, the integration claim was trusted from any issuer: a token signed by one trusted platform could claim another tenant's integration and receive its scopes. Each platform now declares which integration values it may assert; a self-serving platform lists only its own, and the claim is rejected if it is outside that list. The one policy input this requires — which integrations the Data Hub, as a hub, is trusted to vend — is built to the confirmed design (mews, toast, opera-cloud) and flagged in the config and ledger as an owner decision to confirm; it is reversible config, and since the Data Hub is the only active platform today and legitimately vends all three, the binding is mainly defense-in-depth for when the product platforms activate as their own issuers.

This touched the functional chat-endpoint tests, handled carefully: several of them minted tokens from issuers that are inactive or absent in the committed registry and encoded the old "any verified token is served" behavior. Rather than weaken them, each was pointed at a purpose-built test registry where those issuers are active with the correct allow-lists, so every test still asserts exactly what it intended — including the end-to-end mews-vs-toast isolation test, whose "no toast page in a mews trace" assertion is intact and passing. The local embed flow is locked by a new test that the local platform file's allow-lists match exactly what the test-host token route mints.

On the frontend, the CSP hardening from S2 turned out to have been applied to only one of the two consumers of the active-domain list: the iframe frame's page builder still emitted localhost origins and fabricated plaintext-http origins into the postMessage allow-list in production, so the frame would have trusted loopback and http embedders once deployed. The fix extracts a single shared helper for the effective embedder domains and their scheme policy, and routes both the CSP directive and the allow-list through it so they can never diverge again — https-only for real domains outside local/dev, loopback kept only in local/dev, and the fail-closed 403 preserved. Three test gaps were closed alongside: a composed test that a scope-changing token renewal resets the conversation, real base64url (not just padded-standard-base64) coverage for the token decoders, and correction of a stale frontend comment that claimed its env set must match the backend's (no longer true after the backend narrowed its offline set).

Verified independently in one combined gate: backend unit 555→566, full backend DB suite 278→281 passed / 1 xfailed (the ingestion flake passed again), knowledge-base 17 unit + 18 DB, frontend vitest 256→263 and typecheck clean, Playwright embed e2e 4 passed, ruff and pyright clean on the touched source (six pre-existing test-double protocol errors unchanged), boundaries green.

## Phase-4 embed security-hardening pass — repair R5: CI skip-guard + honest deploy-gate status (2026-09-22)

Two low-severity findings on the harness and operator docs. The CI coverage floor counted collected tests, not executed ones, so a mandatory security regression test could be marked skipped and still slip through a green run; and CI ran only on pull requests. R5 adds a zero-tolerance skip-guard to both test steps — the run now fails if any test is skipped, if a strict-xfail unexpectedly passes, or if any new xfail appears other than the single, already-known Planned worker test — and adds mainline (`push` to main) and merge-queue triggers so the gate guards the trunk itself, with branch protection noted as the complementary control. Separately, the migration-0010 deploy gate is honest about its own status: there is no automated deploy pipeline yet, so the runbook now states plainly that the check is a manual step today, exactly where it must be wired as a blocking pre-serve step once a pipeline exists, and adds a clearly-labelled CI smoke step that runs the check against a freshly-migrated local database purely to prove the CLI keeps working — explicitly not the production gate.

Validated locally rather than trusted: the added skip-guard invocation exits zero today for both the non-database and database selections (566 and 281 passing, the one known Planned strict-xfail tolerated, zero real skips), the workflow YAML parses, and the deploy-readiness smoke exits zero against the local store. No production or test code changed.

## Phase-4 embed security-hardening pass — repair R4: consolidate db-test harness + fix the flake (2026-09-22)

The most consequential of the test-infrastructure findings: a safety guard that refuses to run destructive database tests against a non-local host — added after two real live-Supabase incidents — was present in only one of five copy-pasted db-test harnesses. The other four would drop and recreate the schema and provision a known-credential reader role against whatever `DATABASE_URL` happened to resolve to, a live cluster included. The fix consolidates all five duplicated bootstraps into a single shared harness module that runs the guard first, once, before anything destructive, and is itself protected by a test that points the resolver at a fake remote host and confirms it raises before any drop. The per-package test files now just opt in to a database-marker-gated truncate, so the database-free unit tests never touch Postgres.

That consolidation also fixed the intermittent flake, whose root cause turned out to be mechanical and a little alarming: each of the five duplicated session fixtures re-derived the test-database name from the `DATABASE_URL` that a previous fixture had already mutated, so a full run actually created five databases (`…_test`, `…_test_test`, and so on) with five independent schema builds racing on one shared engine, which is what intermittently left a page with no staged chunks. The shared harness derives the name once, idempotently, and builds the schema exactly once — after which only a single test database exists. This first attempt was interrupted partway when the run hit an account spend limit; its half-applied edits were discarded to a clean base and the work redone from scratch, so nothing inconsistent was ever committed.

A deliberate scoping call: per-test isolation stays truncate-based rather than transaction-rollback, because the reader is a separate Postgres role on a separate engine that must see committed data and the production code opens its own sessions — rollback isolation would not model that. The flake was the database-name cascade, not truncation, so removing the cascade fixes it deterministically without a risky rollback rewrite. Verified independently: the full database suite passed twice in the main window (281 each) on top of the subagent's three runs including a reversed package order, only one test database remains afterward, the four guard tests pass, unit is 570 (backend) plus 17 (knowledge-base), and boundaries are clean. This resolves the FLAKE-1 item opened during S3 — root cause found and fixed, not merely observed. All five Wave-1 repair slices (R1–R5) are now complete and committed.

## Phase-4 embed security-hardening pass — Wave-2 re-audit + repairs (2026-09-22)

A second, fully independent audit wave — six fresh agents that neither implemented nor previously audited the code — re-attacked every Wave-1 fix, hunted for regressions the repairs might have introduced, and re-swept for anything new. All the Wave-1 core fixes were independently confirmed to hold, and the parent-fetch scope concern (FINDING-AUTH7) was re-weighed and judged acceptably bounded. But the wave was not clean: it surfaced four real issues, two of them high-severity, which is exactly why two consecutive clean waves are required rather than one.

The most important: the earlier live-database guard fix had been applied only to the backend test harnesses; the knowledge-base db-test suites still created databases and reset the reader-role password against whatever host resolved, with no guard — the same live-data risk, in the tree the earlier fix didn't touch. That is now closed with a knowledge-base-local guard (a copy, since that tree cannot import from the backend), wired as an autouse fixture that runs before any harness touches the database, and proven by forcing a remote URL and watching it refuse before any `CREATE DATABASE`. The second high-severity item was a regression I introduced myself: the CI smoke step added in the previous repair crashed in CI because there is no `.env` there, so the fail-closed default treated it as production and raised on the placeholder registry before the check could run — fixed by giving that step the same explicit test environment every other CI step uses. The two lower-severity items were a loopback check that only matched the exact `localhost`/`127.0.0.1` (now broadened to the whole loopback range including IPv6 `::1`, while still excluding lookalike hostnames) and two stale doc strings that still called `dev` an offline environment.

Verified independently across the combined change: backend unit 570, knowledge-base unit 23, backend DB suite 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 274, typecheck clean, boundaries green. Because Wave 2 was not clean, the clean-wave counter resets — targeted mutation testing and a third audit wave follow, and two consecutive clean waves are still required before the loop can exit.

## Phase-4 embed security-hardening pass — mutation testing + Wave-3 re-audit + repairs (2026-09-22)

Mutation testing deliberately broke each security boundary in turn to prove the guarding test catches it. The first attempt was invalid and thrown away — the isolated worktree was created from the pre-hardening branch, so it never saw the new controls — and it was redone in the main tree on the correct branch. Every boundary's test went red under its mutation with one exception worth having: the web-storage guard only checked reads, so a token written to sessionStorage slipped past. That is exactly the weak test mutation testing exists to find; it was rewritten to assert storage contents directly (and, after a first fix that tripped on this jsdom env exposing only sessionStorage, corrected to a guarded content check) and now catches the write.

The third audit wave — a second fully independent set of agents — confirmed all the earlier fixes hold but again found the same recurring pattern: a fix applied in one place and not its twin. This time the backend trust guard still matched only the exact loopback hosts while the frontend had been broadened, so a production platform whose issuer or key URL was `::1`, `0.0.0.0`, or anywhere in `127.0.0.0/8` would not have been rejected. That is now fixed with the same matcher on both sides. The rest were low: the platform registry was being re-parsed on every request (now cached to one validated instance, keyed so a test's copied settings still recompute), three migration tests shared a database name (now unique so a future parallel runner can't collide), the loader had no test pinning that it never persists the token (added and mutation-verified), and a few stale doc strings — including one, a lint over-length line, that a doc edit of mine in the previous wave had introduced, so it was rewrapped to keep the lint baseline from rising. The parent-fetch scope concern was independently re-weighed a second time and confirmed a non-issue; the optional defense-in-depth was deliberately not taken, since it would change intentional design for a confirmed non-vulnerability, and is recorded as a deferred idea instead.

Verified across the combined change: backend unit 590, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 276, typecheck clean, ruff clean on touched files, boundaries green. Wave 3 was not clean either (one medium plus low items), so the clean-wave counter is still at zero — Waves 4 and 5 must both come back clean.

## Phase-4 embed security-hardening pass — Wave-4 re-audit + repair (2026-09-22)

The fourth wave confirmed every prior fix holds and found two more low, defense-in-depth-only gaps — the tail of a thorough audit, both in the loopback/`.local` host matcher. The backend guard refused a `.local` host as an issuer but not as a frame-ancestor domain (an internal asymmetry of the same guard), and both sides missed a handful of loopback aliases — the trailing-dot form, the fully-expanded IPv6 loopback, and the IPv4-mapped IPv6 form. Neither is a data or scope bypass (access still requires a verified token and no shipped config triggers them), but rather than patch each alias one wave at a time, the matcher was rebuilt comprehensively and identically on both sides — with a small hand-rolled IPv6 expander so there is no new dependency — and checked against a spread of lookalikes to be sure it does not over-match a real public host. The intent is to close the whole class so later waves stop turning up new spellings of "localhost".

Verified: backend unit 616, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 284, typecheck, ruff, and pyright clean, boundaries green. Wave 4 was not clean (two low findings), so the counter is still zero — Waves 5 and 6 must both come back clean.

## SEC-RW6 · Wave-6 re-audit finding W6-5-L1 (loopback twin parity on non-ASCII digits)

The prior session's Wave 6 never really ran — all five audit agents were killed mid-run by an account spend limit, and the workflow's "clean" result was spurious (nothing had executed). This session re-ran a genuine Wave 6 against unchanged code (HEAD 54370d3), after independently re-verifying the whole gate was still green. Five fresh independent auditors attacked the core from the code only, defaulting every suspicion to refuted unless they could reproduce it. Four came back clean with detailed, code-anchored lists of attacks that failed — forged tokens rejected, a 26-case loopback matrix agreeing across the backend and frontend twins, production config fail-closing on the datahub placeholder, database-level RLS isolation proven on the local Postgres, both database-test harnesses guarding before any destructive step, and the CI skip-guard tied down to its single allowlisted xfail. The cross-cut critic agreed the security posture held but found one genuine low-severity defect the domain reviewers missed.

That finding, W6-5-L1: the backend's IPv4 loopback check used Python's `str.isdigit()`, which counts non-ASCII "digits" (Arabic-Indic ٥, fullwidth １, superscript ²) as digits, whereas the frontend twin's `/^\d{1,3}$/` is ASCII-only. So `127.0.0.٥` was treated as loopback by the backend but not the frontend — a divergence between the two matchers, which is exactly the recurring bug the twins were written to prevent — and `127.0.0.²` crashed with an unhandled `ValueError` (`int("²")`) at production-registry load instead of being cleanly rejected. Both effects are low severity and fail-closed: the matcher only runs while validating the operator-edited platforms.json at production boot, never on a request, so it is not attacker-reachable. But a twin divergence violates a stated invariant, so it was fixed.

I confirmed the bug by running the real functions (both the wrong classification and the crash reproduced), wrote the failing tests first (six backend, one frontend), then made the octet checks ASCII-only (`o.isascii() and o.isdigit()`) at both sites in platforms.py so they mirror the frontend regex. A non-ASCII-digit octet is now a normal non-loopback host on both sides, and the matcher never raises. Real hosts and every valid loopback form still classify exactly as before.

Two disclosed deviations: the repair was done in the main window rather than a separate repair subagent (the spend limit kept killing subagents this whole loop; independence still comes from the fresh re-audit waves that follow, and the loopback code's original author was a different subagent, not me), and I added a frontend parity test even though the frontend was already correct, so the invariant is pinned on both twins.

Verified after the fix: backend unit 622, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 285, typecheck, ruff, and pyright clean on touched files, boundaries green. Wave 5 had been clean (consecutive-clean #1), but because Wave 6 surfaced an actionable finding the counter resets to zero — Waves 7 and 8 must both be clean before the loop can exit.

## SEC-RW7 · Wave-7 re-audit findings W7-A1-1 + classify_scope hardening

Wave 7 ran five fresh independent auditors against the post-SEC-RW6 tree, each asked to first prove the loopback fix held and then re-attack the whole core. Three came back clean with strong evidence — one auditor even reproduced the SEC-RW6 revert to prove the six new tests genuinely go red, another re-proved the database-level isolation on the local Postgres, a third tied the CI skip-guard down to its single allowlisted xfail. Two auditors each raised one item, both in the same "Python `isdigit()` accepts characters that then break" family the previous wave was about.

The actionable one, W7-A1-1: the backend loopback matcher `.strip()`s the host it extracts, so a malformed domain like `[ ::1]` (a space inside the brackets) or `127.0.0.1 :80` is treated as loopback; the frontend twin extracted the same host but never re-trimmed, so it treated those as ordinary hosts. That is a divergence between the two matchers — the exact invariant they exist to hold. It is not a live security hole: the backend is the stricter side and refuses such a platform at config load, and `[ ::1]` is not an origin a browser would ever match. But a divergence is a divergence, so I fixed it by trimming the extracted host in the frontend, mirroring the backend, and confirmed both sides now agree that all five malformed forms are loopback.

The second was a completeness-critic note, not a v1 defect: `classify_scope` in the retrieval permission code does the same `isdigit()`-then-`int()` dance, which would crash on a superscript digit. I traced its callers myself and confirmed it cannot happen in v1 — the only live caller passes `auth.principal`, which is always None for embedded users, and a body-supplied principal is both inert and rejected if it is all digits. So it only ever sees None today. Rather than defer it, I applied the same one-line ASCII guard I used for the loopback fix, because it is behaviour-preserving (real space-ids are plain ASCII numbers) and it closes the entire `isdigit`-then-`int` bug class so later waves stop rediscovering pieces of it. I checked the three other `isdigit` sites in the backend and left them alone with reasons: two never call `int()` at all, and the third is an operator tool parsing the database's own version string.

Verified after the fix: backend unit 623, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 286, typecheck, ruff, and pyright clean on touched files, boundaries green. Wave 7 was not clean because of W7-A1-1, so the consecutive-clean counter stays at zero — Waves 8 and 9 must both be clean before the loop can exit. Both repairs were done in the main window rather than a separate repair subagent, the same disclosed deviation forced by the spend limit that keeps terminating subagents; the independence the loop requires still comes from the fresh audit waves that follow.

## SEC-RW8 · Wave-8 finding W8-A1-1 — closing the loopback-twin whitespace class

Waves 6 and 7 had each found one more obscure way the backend loopback matcher disagreed with its frontend twin, so for Wave 8 I told the auditors to stop finding leaves and go for the root: two of them ran a large differential that actually executes both real implementations — the Python backend and the real csp.ts under Node — over thousands of generated hosts and reports every input the two classify differently. They did, and they independently landed on the same answer.

The root cause, W8-A1-1: Python's `str.strip()` and JavaScript's `String.prototype.trim()` do not remove the same set of edge whitespace. They differ on exactly six code points — Python also strips the C1 field separators U+001C–U+001F and the NEL U+0085, and JavaScript also strips the byte-order mark U+FEFF. So a domain like `localhost` with a trailing BOM was treated as loopback by the frontend (stripped and matched) but not by the backend (kept, so not matched) — meaning it could slip past the backend's production trust guard while the frontend would have stripped it. Two independent fuzzers, one running 3,779 inputs and the other 3,137, both concluded the entire disagreement reduces to those six characters and nothing else, which told me the divergence set was complete and I could close it for good rather than patch another character.

The fix is deliberately the whole class, not the six characters: both sides now strip one shared, explicitly enumerated union set of every code point either runtime strips — a `_strip_edges` helper on the backend and a matching `stripEdges` on the frontend — used everywhere a host is normalized. The set is exactly the union, so U+200B (a zero-width space that neither runtime strips) is deliberately left out, and I pinned that with a test so the set can't quietly grow. To confirm the twins are now truly equal I ran my own differential: every whitespace, control, and format code point up to U+FFFF, prepended and appended and bracketed onto ten different loopback and non-loopback bases — 3,993 inputs — through the real backend and the real frontend. Zero disagreements, where the same corpus showed 149 before the fix. The separate enumeration of every `isdigit`/`int()` call site also came back clean, so with both that class and this one closed by construction, the loopback-parity tail should finally be exhausted.

Verified after the fix: backend unit 630, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 288, typecheck, ruff, and pyright clean on touched files, boundaries green. Wave 8 was not clean because of W8-A1-1, so the consecutive-clean counter stays at zero — Waves 9 and 10 must both be clean. As with the two prior repairs, this was done in the main window rather than a separate repair subagent, the same disclosed spend-limit deviation, with independence coming from the fresh audit waves that follow.

## Phase-4 embed security loop — EXIT (two consecutive clean waves)

Waves 9 and 10 both came back clean, which meets the mandate's exit condition of two consecutive fresh independent audit waves with no actionable finding, so the implement-audit-repair loop is complete for the locally-provable core.

The last three not-clean waves (6, 7, 8) were all the same story in the same place: the backend and frontend each have a copy of the "is this host a loopback/.local address that must be stripped from production trust" logic, and the two copies kept disagreeing on progressively more obscure inputs — first non-ASCII digits, then whitespace tucked inside brackets, then the six characters where Python's strip and JavaScript's trim simply remove different things. None was a real exploit: the backend is the stricter side and refuses a bad config at load, and the inputs have to be hand-typed into operator config, never sent by a user. But a divergence between the two copies is exactly the bug the code was written to prevent, so each was fixed with a failing test first. The final fix stopped patching individual characters and instead made both copies strip one shared, explicitly listed set of characters, which closes the whole class: my own differential run of nearly four thousand inputs went from 149 disagreements to zero, and the Wave 9 and 10 auditors independently confirmed zero across hundreds of thousands more.

Wave 9's only note was a suggestion to add a permanent cross-language differential test to CI so the two copies can never silently drift again. The auditor who raised it called it explicitly "not a defect," the completeness-critic auditor separately concluded there was no missing test, and the current parity is proven across roughly 882,000 inputs with non-vacuous per-side tests, so I judged it a non-actionable hardening idea and recorded it in future-ideas.md for the owner to weigh rather than treating it as a finding. Wave 10 then re-audited the whole core from scratch — four fresh auditors across auth, browser, isolation, and config/CI — and found nothing actionable, confirming the second clean wave.

I wrote the mandate's full final report to FINAL-REPORT.md in the phase-4 folder: every round and its agents, every finding and whether it was confirmed or disproven with evidence, the repairs and their regression tests, the mutation results, the final test commands and numbers, the requirement traceability, the two clean results, and — kept carefully separate — everything still required before anyone may claim real Base or Data Hub or production readiness (real issuers, JWKS, domains, token endpoints, host keys, a datahub handover pack, and migration 0010 applied in pre-production). The word "done" here means the local core only.

Verified at exit (HEAD 0905ad2): backend unit 630, knowledge-base unit 23, backend DB 281 passed / 1 xfailed, knowledge-base DB 18, frontend vitest 288, Playwright embed e2e 4, typecheck, ruff, pyright clean on touched files, boundaries green.

## SEC-BRIEF · operator runbooks + localhost test enablement (2026-09-22)

The owner asked, after the embed-security loop closed, for the practical follow-on: split the remaining work into "needs Base/Data Hub people" vs "things we can do now," make the localhost embed test reproducible on our own machine, create the brief folder the mandate keeps referencing (placed at docs/Final_docs/brief per the owner's request), and write tests. This entry records that work.

Three research agents mapped the real code first (how an integration is added, how a host platform is added, how the localhost embed test actually runs) so the runbooks carry accurate file:line links rather than guesses. Two useful facts fell out: the host key is a single global CHAT_API_KEY (there is no per-platform key — a mandate line that does not map to this architecture), and the existing local-test-keys doc has stale apps/web paths from an older layout.

I created docs/Final_docs/brief/ with four files: a README carrying the Bucket-1 (external) / Bucket-2 (local) split, add-an-integration.md (a pure two-config-file change plus Confluence labels), add-a-host-platform.md (config-only now vs. the four real values that must come from the platform's developers before active:true is safe in production), and localhost-test.md (the corrected recipe). The localhost recipe fixes a real footgun the research found: the local platform registry pins JWKS on :3100, but `make web-dev` serves :3000, so a person following the old instructions got silent 401s. To make it push-button I added two Makefile targets — `make api` (backend on :8000) and `make embed-dev` (frontend on :3100 with the scope switcher) — since no backend run target existed at all.

For tests: a config-layer isolation regression pins that the committed platforms.local.json resolves each test integration to exactly its own scope plus general and never another integration's scope (the cross-integration isolation the localhost test demonstrates, proven without needing a browser), and a doc-integrity test extracts every backtick repo path from the brief runbooks and asserts each resolves — so moving the referenced code rots the test, not a reader's afternoon, and the clean-exit "docs/brief runbooks match implemented code" gets an automated guard. Backend unit went 630 → 633; boundaries, ruff, format and pyright are clean on the touched files.

Finally, docs/future-ideas.md gained an "Integration with other software — putting Obi live in a host platform" section: what we can already do, what we change on our side to go live, what must come from the external platform's developers, and what is deliberately out of scope for v1. Honest scope notes: this slice is docs + unit tests + make targets only (no product code path changed, so no DB/e2e/eval level was run), and it documents rather than closes the two deeper localhost gaps (no offline vector seed for the test scopes; no token-mint CLI), which stay in future-ideas.

## SEC-EMBED-FIX · /embed server-only build break (2026-09-22)

While the owner was testing the widget on localhost, the embed frame failed to load: `/embed` returned 500 with "You're importing a component that needs server-only." I debugged it systematically rather than guessing.

The import trace was the whole story: the chat feature's client public root (`src/features/chat/index.ts`) re-exported the server-only proxy handlers (`handlePostChat`/`handlePatchFeedback` from `./server/route-handlers`), and that proxy reaches `@/platform/automation-api` → `client.ts` → `import "server-only"` (the module that injects the `CHAT_API_KEY` host secret). The `/embed` client component (`embed-frame.tsx`, a `"use client"` module) imports that barrel for its UI and session-hook exports — so Next pulled the whole barrel's module graph, server-only included, into the browser bundle and failed the build. The widget's Playwright mount test never caught it because it only checks that the `<iframe src="/embed">` tag exists on the parent page; it never loads `/embed` itself, and vitest/tsc don't compile the client/server split.

The codebase already had the fix pattern documented one feature over: `features/embed/index.ts` deliberately keeps its server-only `platforms.ts` out of the client barrel and makes server consumers import it directly. I mirrored that for chat: removed the server re-export from the client root, added a dedicated `src/features/chat/server.ts` server root (carrying its own `import "server-only"`), and pointed the two API route entrypoints at `@/features/chat/server`. The `route-handlers` wiring test's mock was retargeted to the server root to match.

I wrote the regression test first and watched it fail: `client-root-no-server-imports.test.ts` asserts the chat client root never re-exports a `./server` module (red before the fix, green after). Then I verified the real thing: full frontend vitest 288 → 289, tsc clean, and a live `curl http://localhost:3100/embed` now returns HTTP 200 with no server-only error in the dev log. The `CHAT_API_KEY` boundary is not weakened — it's more strongly isolated now, since the client root no longer references server code at all. Honest note: the live proof is a curl against the running dev server; the committed regression is the fast unit guard, and strengthening the Playwright mount test to actually load `/embed` is a sensible follow-up.

## SEC-ENV · root .env restructure + LOCAL/LIVE switch + Anthropic key (2026-09-23)

The owner asked to reorganize the gitignored root `.env` so it reads by purpose ("this is for that"), to make going live a matter of flipping a few clearly-marked lines rather than hunting through the file, and to set the Anthropic API key. I did all three without changing any existing value — pure re-layout, comments, and one new key.

The file is now grouped into labelled sections (Deployment switch, Database, Confluence, Models, Embeddings, Contextual retrieval, Reranker, Chat host key, Budgets, Reconciliation, Knowledge scopes, Runtime), with a header that explains the LOCAL-vs-LIVE convention once: only a handful of vars differ between localhost and live, they sit in a "DEPLOYMENT SWITCH" block at the top (ENV, PLATFORMS_PATH), and each environment-specific value elsewhere (the two Supabase DSNs) carries a commented `# LIVE:` line beneath it. Going live becomes: comment the LOCAL line, uncomment and fill the LIVE line, flip ENV to production last — no code edit anywhere, because the platform registry and every URL are already data/env-driven.

I set `ANTHROPIC_API_KEY`, which had been absent — meaning answer generation and per-chunk contextualization were both silently running on the deterministic Fake provider. With the key set, those now make real Anthropic calls (the behavior change the owner intended). The pasted key was written only into the gitignored `.env` (`.gitignore:20`), never into any committed file, doc, or log.

One thing I flagged rather than fixed, to honor "don't change anything": the `DATABASE_READER_URL` DSN contains a space between the password and `@aws-1-...` (`...3475I @aws-...`). Inside a quoted connection string that space is literal and will break the non-owner reader connection if it wasn't intended — worth verifying before go-live, since the app refuses to boot in a non-offline env without a working reader role.

Verification was a byte-level preservation check rather than a test run (the suite doesn't read `.env`, and no product code changed): 42 assignment lines = the 41 originals, each grep-confirmed unchanged, plus the one new key. No test level applies to a gitignored config file. This is owner-requested go-live prep, not a numbered action-plan substep.

## SEC-TUNNEL · one-command "test online" tunnel for the embed widget (2026-09-23)

The owner wanted a fast way to see the widget working on a public URL (just for themselves, no partner, general-only content). I added `scripts/tunnel-embed-test.sh` and a `make tunnel` target that stands up a cloudflared quick tunnel over the local frontend and prints a public HTTPS link to open.

The design keeps the exposure minimal: only the Next.js frontend (:3100) is tunneled. The browser only ever talks to the frontend, and the frontend calls the FastAPI backend server-to-server on localhost:8000 (via AUTOMATION_API_BASE_URL), so the backend never needs to be public. The one real obstacle is that Obi's embed defenses — the iframe's CSP `frame-ancestors` and the postMessage origin allow-list — are both built from the *active platform domains* in platforms.local.json, and dev mode adds no wildcard and no 'self'. An un-listed tunnel host would silently fail: the iframe wouldn't load and the open message would be rejected, leaving a blank panel. So the script injects the tunnel's (random) hostname into every active test entry's domains for the session, and because the frontend re-reads that file per request, it takes effect with no restart and no obi.js rebuild (the loader already falls back to the page's own origin when OBI_ORIGIN is unset). On Ctrl-C it kills the tunnel and restores the registry file verbatim from a snapshot.

For the general-only experience it points you at the existing /test-hosts/none page, which mints a token with no business claims; the panel opens even if no token is present, and the backend treats a missing token as the general-only path — so you get answers without any partner or real integration.

I verified the mechanics rather than the whole browser flow: the script parses (`bash -n`), the JSON domain-injection round-trips correctly on a copy (all four active entries gain the host, valid JSON out), `make -n tunnel` resolves, and — the trickiest part — a real cloudflared quick tunnel issued a live URL that the script's exact capture regex picked up (`https://cross-labor-infants-designed.trycloudflare.com`). The real platforms.local.json was confirmed unchanged by the tests. One thing the verification caught and fixed along the way: macOS has no GNU `timeout`, so the script avoids it and uses a bounded poll loop with a liveness check instead.

Honest scope: I did not run the full end-to-end load (open the tunnel URL in a browser and get an answer) — that needs `make api` and `make embed-dev` running plus a person at the browser, which is the owner's next step, and a non-empty general-only answer also depends on there being obi-general-test content in the live corpus. This is owner-requested go-live tooling, not a numbered action-plan substep, and it's in the working tree (not yet committed).

## OBI-PROMPT · one general Obi system prompt + per-chat company/integration (2026-09-23)

The owner asked for one general system prompt that defines how Obi responds — clear, helpful, professional, concise, grounded in cited sources — and, for each chat, to add the business name (`company_name`) and integration (`integration`) from the verified server-side JWT context so Obi knows which business it is helping and which platform they use, without guessing either if it is missing, and without touching document access (which stays enforced by retrieval and the database).

I did two things in the rag_agent domain (the only place prompt text is allowed to live). First, I reframed `ANSWER_SYSTEM_PROMPT` to open by naming Obi and its response qualities — "You are Obi, a professional, helpful, and concise support assistant. You answer clearly and ONLY from the numbered evidence blocks provided…" — while preserving every load-bearing citation and grounding sentence verbatim (the citation contract `enforce_citations` depends on is untouched, and the existing tests that assert those substrings still pass). Second, I added `build_answer_context_block(company_name, integration)`, which renders the verified business context as a second, uncached system block on the grounded answer call. It carries `AnthropicAnswerGenerator.generate`'s new keyword-only `company_name`/`integration`, threaded from the verified `AuthContext` in `answer_service.py` — never from the request body.

Two deliberate design choices, both mirroring the existing identity path. The business context rides a *separate, uncached* block so per-chat company/integration variation never busts the cached `ANSWER_SYSTEM_PROMPT` prompt cache. And a missing value is simply omitted — when the token carries neither company nor integration (the general/tokenless path), the block returns `None` and nothing is added, so Obi never invents a business. The values are presented as "facts, not instructions" (the same anti-injection framing the identity block uses) and are not redacted, because they are verified token claims, not user PII. Nothing about access changed: this is response context only.

I wrote the tests alongside the change: four for the prompt builder (renders both, omits a missing one, returns None when neither is known, and the persona/qualities are present), two for the generator (the uncached second block is attached when context exists, and absent when it does not), and one for the service (the verified company/integration are threaded into `generate`; the tokenless path threads `None`). 100 passed across the three touched files, 264 across the whole rag_agent unit suite; ruff + pyright clean on touched files; `make boundaries` OK. Honest scope: I did not run the DB-marked tests (local Postgres wasn't up, and this change touches no DB behavior) or `make eval` — a generation-prompt tweak can shift answer quality, so a gold-set before/after is worth running before this goes live. The design page `r5-generate` panel and the generated brief were updated to match. Not committed.

## OBI-WIDGET · embed panel fills the frame + dev scope switcher removed (2026-09-23)

Reviewing the running embed, the owner pointed out two things: a thin (~5px) vertical bar on the left of the panel next to the "Obi" mark, which should go away so the header is flush; and the header "tag" icon, which opened a menu to pick General/Muse/Toast that they did not recognize and wanted removed entirely unless it was crucial.

The left bar came from how the panel sits inside its iframe. obi.js sizes the embed iframe to 380px and already draws its rounded corners and shadow, but inside it `FloatingFrame` pinned itself to the right at a clamped 360px width with a `border-l` line — leaving a ~20px empty strip on the left plus a border seam inside the rounded corner. I gave `FloatingFrame` a `fill` variant that drops the width clamp, the right-pin, the left border, and the shadow, and had the `/embed` frame use `<FloatingFrame fill>` so the panel fills the iframe edge-to-edge. The main-site docked preview keeps the default. Now the header sits flush with only its normal 16px content padding.

The tag icon was the dev-only knowledge-scope switcher (PLAN 10.8) — never shown in a real embed, and appearing only because the `make embed-dev` target force-enabled `NEXT_PUBLIC_SHOW_SCOPE_SWITCHER`. Since it is not a product feature, I removed it from the code entirely: deleted `scope-menu.tsx`, its model `model/knowledge-scopes.ts`, and their tests (`scope-menu.test.tsx`, `knowledge-scopes.test.tsx`); deleted the `e2e/scope-list.spec.ts` browser test and removed the flag from both the Makefile `embed-dev` target and `playwright.config.ts`; and stripped the switcher wiring (button, menu, state) from `panel-header.tsx`. I deliberately kept the session-context scope plumbing (`knowledgeScope`/`setKnowledgeScope` in `chat-session-provider.tsx`) — that is the request-body field, unset in the embed, and removing it would touch unrelated request/permission plumbing the owner did not ask about.

Verification: 192 tests pass across the chat + embed vitest suites (including a new test asserting the `fill` variant has no left seam or clamped width), `tsc --noEmit` is clean, and there are no dangling references to any deleted file. Honest note: the dev switcher was a real, documented scope-isolation verification tool, so removing it drops that in-widget check — the backend's scope validation and isolation are unaffected. I also could not run the project's `pnpm lint` (its `next lint` drops to an interactive setup prompt with no committed eslint config — pre-existing), but the typecheck is clean. The design page (`w-scope`, `ov-widget`, `cm-config`, `ks-widget`, the scope-list panels, and two prose bits, all of which referenced the removed switcher or the deleted `knowledge-scopes.ts`) was updated and the brief regenerated. Not committed.
