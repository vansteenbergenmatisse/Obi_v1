...
- [x] **Live isolation run** — done 2026-09-16 against staging (owner-confirmed), output saved to `docs/Final_docs/0.5-regression-tests/live-isolation-2026-09-16.txt` (exit 0, all isolation checks green). 11 "needs live" items from delta.md checked (12 rows, two grouped by panel-set): 6 green, 6 red (one open line per red item added to docs/plan/decisions.md). 0.5.3 fully closed: all 13 test-writing panels (Batches A-D) + the live run.
...
| `cm-docs` | Does an ADR exist for fingerprint/scope_state/label-gated-ingestion/edge-token? | `ls docs/adr/` (local, not DB) | red | Checked all 13 ADRs on disk (0001-0014, no 0012); none is titled or scoped to these four topics. No such ADR exists today. **[Updated 2026-09-18: ADR-0015 (fingerprints), ADR-0016 (scope_state as tags — with a divergence caveat), and ADR-0017 (edge-token) are now written; the 4th, label-gated ingestion, is decision-needed (code diverges from rule #5). See `docs/plan/decisions.md` `live-0.5.3-cm-docs` and `docs/plan/delta.md`. The "red" above reflects the 2026-09-16 live run.]** |
