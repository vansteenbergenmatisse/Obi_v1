# Obi embed — Toast hand-over pack

**Phase 11.1c (ADR-0014).** Everything Toast needs to embed Obi, and the four values we need back
to switch it on. Until those four are filled in `config/platforms.json` and `active` is flipped to
`true`, the `toast` entry stays inactive: the frame's CSP won't list your domain and the loader
...
| `integration` | **`toast`** (the only value that maps to knowledge scope) |
...
export async function mintObiToken(user, company) {
  ...
  return new SignJWT({ company_id: company.id, company_name: company.name, integration: "toast" })
    .setProtectedHeader({ alg: "RS256", kid: "obi-toast-1" })
    .setIssuer("https://pos.toasttab.com").setAudience("obi").setSubject(user.id)
...
def mint_obi_token(user_id: str, company_id: str, company_name: str) -> str:
    now = int(time.time())
    return jwt.encode(
        {"iss": "https://pos.toasttab.com", "aud": "obi", "sub": user_id, "iat": now, "exp": now + 3600,
         "company_id": company_id, "company_name": company_name, "integration": "toast"},
        OBI_PRIVATE_KEY_PEM, algorithm="RS256", headers={"kid": "obi-toast-1"},
    )
```
...
2. A signed-in user gets answers scoped to **Toast + general** content — never another platform's.
...
1. **Issuer** — the `iss` you sign with (hint: `https://pos.toasttab.com`).
2. **JWKS URL** — where your public keys live (e.g. `https://pos.toasttab.com/.well-known/obi-jwks.json`).
3. **Browser domain(s)** — where the widget runs, for the frame's `frame-ancestors` CSP
   (hint: `pos.toasttab.com`).
4. **Integration value** — confirm it is `toast` (maps to the `obi-toast-test` knowledge scope).

Also: one test company on Toast and one signed-in test user, so we can prove isolation end-to-end.

## 5. The one line that activates Toast

In `config/platforms.json`, fill the `toast` entry's `issuer` / `jwks_url` / `domains` from §4 and
flip:
...
Validated at startup: the mapped scope must exist in `config/knowledge_scopes.json` (`obi-toast-test` does),
