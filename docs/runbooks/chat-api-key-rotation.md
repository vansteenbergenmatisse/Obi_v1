...
`CHAT_API_KEY` is the shared secret between `frontend`'s server-side chat proxy
(`platform/automation-api`) and `backend`'s `POST /chat` / `PATCH /chat/{traceId}/feedback`
...
The key lives in two separate `.env` files (root `.env` for `backend`, `frontend/.env.local`
...
- The web proxy only ever sends **one** key (`CHAT_API_KEY` in `frontend/.env.local`) — it never
  needs the previous value; only the verifying side (`backend`) needs to accept both.
...
1. **Rotate in the new key** (from `backend`):
   ...
   `frontend/.env.local`, and moves the *old* value into `CHAT_API_KEY_PREVIOUS` (still accepted).
...
4. **Close the window** once every caller (in practice: just the one `frontend` proxy today) is
   ...
   This blanks `CHAT_API_KEY_PREVIOUS` — the old key stops working. Restart `backend`
